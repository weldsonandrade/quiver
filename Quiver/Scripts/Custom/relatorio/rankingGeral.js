...
function exibirExcecaoSelecionada() {
    exibirExcecaoSelecionadaRelatorio('ranking', 'DDLFilterRankingUsuario_chosen', 'DDLFilterRankingUnidade_chosen', 'DDLFilterRankingGrupo_chosen');
}
        ...
        formatters: {
            "efetividade": function (column, row) {
                ...
                if (efetividade == 0.0) {
                    ...
                    return 'Sem inspeções finalizadas ou com pontos.';
                }
            ...
            "efetividade2": function (column, row) {
                var efetividade = row.efetividade;
                if (efetividade == 0.0) {
                    return 'Sem inspeções finalizadas para calcular eficiência.';
                }
                if (efetividade.substr(efetividade.indexOf('.')) == ".00") {
                    return efetividade.substr(0, efetividade.indexOf('.'));
                }
...
function filtrar() {
    ...
    $('#titulo-Ranking').html("Ranking de <strong id='categoria-Ranking-ID'>" + categoriaSelecionada + "</strong>  por  <strong id='tipo-Ranking-ID'>" + tipoRanking + "</strong> das inspeções finalizadas entre <strong>" + dtInicial + "</strong> e <strong>" + dtfinal + "</strong> <small id='sub-titulo-Ranking'>.</small>");
    ...
    if (($('#dataInicial').val().trim() == "") && ($('#dataFinal').val().trim() == "")) {
        ...
        // Escreve o titulo
        $('#titulo-Ranking').html("Ranking de  <strong id='categoria-Ranking-ID'>" + categoriaSelecionada + "</strong> por <strong id='tipo-Ranking-ID'>" + tipoRanking + "</strong> das inspeções finalizadas durante todos os tempos. <small id='sub-titulo-Ranking'>.</small> ");
    }
    else if (($('#dataInicial').val().trim() != "") && ($('#dataFinal').val().trim() == "")) {
        dtfinal = moment().add(7000, 'year').format('DD/MM/YYYY');
        $('#titulo-Ranking').html("Ranking de  <strong id='categoria-Ranking-ID'>" + categoriaSelecionada + "</strong> por <strong id='tipo-Ranking-ID'>" + tipoRanking + "</strong> das inspeções finalizadas que se iniciarem em: <strong>" + dtInicial + "</strong>  <small id='sub-titulo-Ranking'>.</small>");

    } else if (($('#dataInicial').val().trim() == "") && ($('#dataFinal').val().trim() != "")) {
        dtInicial = moment().subtract(500, 'year').format('DD/MM/YYYY');
        $('#titulo-Ranking').html("Ranking de  <strong id='categoria-Ranking-ID'>" + categoriaSelecionada + "</strong> por <strong id='tipo-Ranking-ID'>" + tipoRanking + "</strong> das inspeções até: <strong>" + dtfinal + "</strong>  <small id='sub-titulo-Ranking'>.</small>");
    }
        ...
        $(".flot-chart").bind("plothover", function (event, pos, item) {
            if (item) {
                ...
                if (item.series.avaliacoesPonto[item.dataIndex][0].length > 1) {
                    ...
                    $(".flot-tooltip").html("Nenhuma inspeção nessa data.");
            }
...
var options = {
    ...
    xaxes: [{ axisLabel: 'Datas das inspeções', }],
...
var optionsQuantidade = {
    ...
    xaxes: [{
        axisLabel: 'Datas de fianlização das inspeções',
    }],
    yaxes: [{
        position: 'left',
        axisLabel: 'Quantidade de Inspeções',
    }],
